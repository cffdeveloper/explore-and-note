...
import { Link as LinkIcon, Paperclip, Trash2, Upload, ExternalLink, FileText, Image as ImageIcon, Save, Loader2 } from "lucide-react";
...
export function ItemCard({ itemId, label }: { itemId: string; label: string }) {
  ...
  // Quick check whether this item has anything saved (for badge)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [{ data: n }, { count }] = await Promise.all([
        supabase.from("item_notes").select("content").eq("item_id", itemId).maybeSingle(),
        supabase.from("item_attachments").select("*", { count: "exact", head: true }).eq("item_id", itemId),
      ]);
      if (cancelled) return;
      setHasContent(!!(n?.content && n.content.trim()) || (count ?? 0) > 0);
    })();
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      setSaving(false);
      setSavedAt(Date.now());
      setHasContent(!!note.trim() || attachments.length > 0);
    }, 600);
    return () => { if (saveTimer.current) clearTimeout(saveTimer.current); };
  }, [note, loaded, itemId, attachments.length]);
  ...
  return (
    <div className="border border-border rounded-lg bg-card/40 hover:bg-card/70 transition-colors overflow-hidden">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-start gap-3 px-4 py-3 text-left"
      >
        <span className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 ${hasContent ? "bg-gold" : "bg-border"}`} />
        ...
        <div className="border-t border-border p-4 space-y-4 bg-background/40">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Write your thoughts, questions, lessons learned…"
              className="w-full min-h-32 bg-input border border-border rounded-md p-3 text-sm font-sans focus:outline-none focus:ring-1 focus:ring-gold resize-y"
            />
          </div>
          ...
          <div>
            <label className="text-[10px] uppercase tracking-widest text-gold flex items-center gap-1.5 mb-2">
              <Paperclip size={11} /> Attach Files / Images
            </label>
            <label className="flex items-center justify-center gap-2 border border-dashed border-border rounded-md py-4 text-sm text-muted-foreground hover:border-gold hover:text-foreground cursor-pointer transition-colors">
